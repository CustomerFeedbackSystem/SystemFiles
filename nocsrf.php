...
class NoCSRF
{
    ...
    public static function check( $key, $origin, $throwException=false, $timespan=null, $multiple=false )
    {
        if ( !isset( $_SESSION[ 'csrf_' . $key ] ) )
            if($throwException)
                throw new Exception( 'Error! Invalid Link' );//Missing CSRF session token.
            else
                return false;
            
        if ( !isset( $origin[ $key ] ) )
            if($throwException)
                throw new Exception( 'Error! Invalid Link' );//Missing CSRF form token.
            else
                return false;
        ...
        {
            if($throwException)
                throw new Exception( 'Error! Invalid Link' );//Form origin does not match token origin.
            else
                return false;
        }
        ...
        if ( $origin[ $key ] != $hash )
            if($throwException)
                throw new Exception( 'Error! Invalid Link' );//Invalid CSRF Token
            else
                return false;
        ...
        if ( $timespan != null && is_int( $timespan ) && intval( substr( base64_decode( $hash ), 0, 10 ) ) + $timespan < time() )
            if($throwException)
                throw new Exception( 'Error! Expired Link' );//CSRF token has expired.
            else
                return false;
