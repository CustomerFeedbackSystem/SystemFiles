...
function requests_man($settings){
	if(isset($_REQUEST['height'])&&($_REQUEST['height']>0))	$settings['height']	= PHPWS_Text::parseInput($_REQUEST['height']);
	else													$settings['height']	= "auto";
	if(isset($_REQUEST['width'])&&($_REQUEST['width']>0))	$settings['width']	= PHPWS_Text::parseInput($_REQUEST['width']);
	else													$settings['width']	= "auto";
	if(isset($_REQUEST['lang_activ']))			$settings['lang_activ']		= 1;
	else										$settings['lang_activ']		= 0;
	if(isset($_REQUEST['view_anon']))			$settings['view_anon']		= 1;
	else										$settings['view_anon']		= 0;
	if(isset($_REQUEST['view_user']))			$settings['view_user']		= 1;
	else										$settings['view_user']		= 0;
	if(isset($_REQUEST['request_mode']))		$settings['request_mode']	= 1;
	else										$settings['request_mode']	= 0;
	if(isset($_REQUEST['enable_css']))			$settings['enable_css']		= 1;
	else										$settings['enable_css']		= 0;
	if(isset($_REQUEST['theme']))				$settings['theme']			= PHPWS_Text::parseInput($_REQUEST['theme']);
	else										$settings['theme']			= "none";
	if(isset($_REQUEST['path']))				$settings['path']			= PHPWS_Text::parseInput($_REQUEST['path']);
	else										$settings['path']			= "";
	if(isset($_REQUEST['browsers']))			$settings['browsers']		= PHPWS_Text::parseInput($_REQUEST['browsers']);
	else										$settings['browsers']		= "none";

	if($settings['plugins']<>'none') {
		$i = 1;
		foreach($settings['plugins'] as $key => $value) {
			if(isset($_REQUEST["plug_$i"]))		$settings['plugins'][$key]	= 1;
...
function manageEditors() {
	if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg", "settings")) {
		$this->content .= "<a href=\"".$this->linkRef."&amp;action=admin"."\">".$_SESSION['translate']->it("Back to administration")."</a>";
		if(isset($_REQUEST['updList']))	//to update, simply delete the existing file
...
function addEditor() {
	if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg","settings")) {//Administrativ condition
		if(isset($_REQUEST['addEditor']))	$data['editor'] = PHPWS_Text::parseInput($_REQUEST['addEditor']);
		else	return FALSE;
		if(isset($_REQUEST['fn']))			$file = PHPWS_Text::parseInput($_REQUEST['fn']);
		else	return FALSE;
...
function delEditor() {
	if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg","settings")) {//Administrativ condition
		if(isset($_REQUEST['delEditor']))	$editor = PHPWS_Text::parseInput($_REQUEST['delEditor']);
		else	return FALSE;
...
function cpyEditor() {
	if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg","settings")) {//Administrativ condition
		if(isset($_REQUEST['cpyEditor']))	$editor = PHPWS_Text::parseInput($_REQUEST['cpyEditor']);
		else	return FALSE;
...
function delFile() {
	if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg","settings")) {//Administrativ condition
		if(isset($_REQUEST['delFile']))	$filename = PHPWS_Text::parseInput($_REQUEST['delFile']);
		else	return FALSE;
		if(isset($_REQUEST['filetyp']))	$filetyp = PHPWS_Text::parseInput($_REQUEST['filetyp']);
		else	return FALSE;
		if($filename) {
			if ($filetyp=="tgz") {
...
function tarEditor() {
	if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg","settings")) {//Administrativ condition
		if(isset($_REQUEST['tarEditor']))	$package = PHPWS_Text::parseInput($_REQUEST['tarEditor']);
		else	return FALSE;
		...
		if(!isset($_REQUEST['local']))	//if not local, copy first
