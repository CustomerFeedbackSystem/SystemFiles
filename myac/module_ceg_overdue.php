          		...
          		<table border="0" width="100%" cellpadding="2" cellspacing="0">            	
                    ...
                    <tr>
						...
						if($_SESSION['dur']=='UPTO3')
							...
							WHERE (TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW())>(tktcategory.tat/60)	
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW()) <=(129600+(tktcategory.tat/60)))	
						...
						if($_SESSION['dur']=='UPTO6')
							...
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory
							LEFT JOIN loctowns ON tktin.loctowns_idloctowns=loctowns.idloctowns
							WHERE (TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW())>(129600+(tktcategory.tat/60))	
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW()) <=(259200+(tktcategory.tat/60)))								
							AND tktstatus_idtktstatus<4 ".$qry."";
						...
						if($_SESSION['dur']=='UPTO9')
							...
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory
							LEFT JOIN loctowns ON tktin.loctowns_idloctowns=loctowns.idloctowns
							WHERE (TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW())>(259200+(tktcategory.tat/60))	
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW()) <=(388800+(tktcategory.tat/60)))								
							AND tktstatus_idtktstatus<4 ".$qry."";
						...
						if($_SESSION['dur']=='UPTO12')
							...
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory
							LEFT JOIN loctowns ON tktin.loctowns_idloctowns=loctowns.idloctowns
							WHERE (TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW())>(388800+(tktcategory.tat/60))	
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW()) <=(518400+(tktcategory.tat/60)))								
							AND tktstatus_idtktstatus<4 ".$qry."";
						...
						if($_SESSION['dur']=='BEYOND12')
							...
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory
							LEFT JOIN loctowns ON tktin.loctowns_idloctowns=loctowns.idloctowns
							WHERE TIMESTAMPDIFF(MINUTE,tktin.timereported,NOW()) >(518400+(tktcategory.tat/60))								
							AND tktstatus_idtktstatus<4 ".$qry."";
                        ...
                        <strong>
                       	...
                       	<td valign="top" class="tbl_data">
							...
							$sql_staff="SELECT fname,lname,usrrolename FROM wftasks 
							INNER JOIN usrac ON wftasks.usrac_idusrac=usrac.idusrac
							INNER JOIN usrrole ON wftasks.usrrole_idusrrole=usrrole.idusrrole
							WHERE tktin_idtktin=".$fet_ticket['idtktinPK']." AND (wftskstatustypes_idwftskstatustypes=0 OR wftskstatustypes_idwftskstatustypes=6)
							ORDER BY idwftasks DESC LIMIT 1";
							$res_staff=mysql_query($sql_staff);
							$fet_staff=mysql_fetch_array($res_staff);
							$num_staff=mysql_num_rows($res_staff);
							
							if ($num_staff>0)
								{
								echo $fet_staff['fname']." ".$fet_staff['lname']." <span style=\"font-size:9px;\">( ".$fet_staff['usrrolename']." )</span>"; 
								} else { ?>
								<span style="color:#FF0000; font-style:italic; font-size:10px;">Not Specified</span>
                                <?php
								}
							?>                        
                        </td>
