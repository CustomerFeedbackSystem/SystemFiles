...
$query_rs_list = "SELECT DISTINCT wftasks.tktin_idtktin,idwftasks,wftasks.tasksubject,wftasks.timeinactual,wftasks.timedeadline,wftasks.wftskstatustypes_idwftskstatustypes,wftasks.timeactiontaken,usrrole.usrrolename as sender_role,sender_idusrrole,TIMESTAMPDIFF(MINUTE, NOW(), wftasks.timedeadline) AS time_to_deadline ,usrac.utitle,usrac.lname,tktin.refnumber,tktin.senderphone,tktin.tktcategory_idtktcategory FROM wftasks 
...
WHERE wftasks.usrrole_idusrrole=".$fet_memberdetails['usrrole_idusrrole']." 
AND wftasks.usrac_idusrac=".$fet_memberdetails['idusrac']." AND wftasks.timeactiontaken='0000-00-00 00:00:00' 
AND wftasks.timedeadline<='".$timenowis."' AND 
(
(wftskstatusglobal_idwftskstatusglobal=2 AND wftskstatustypes_idwftskstatustypes=6 ) 
OR 
(wftskstatusglobal_idwftskstatusglobal=1 AND wftskstatustypes_idwftskstatustypes=0 ) 
)
...
?>>
		...
		?>        </td>
		<td class="tbl_data" <?php if ($row_rs_list['wftskstatustypes_idwftskstatustypes']==0) { echo "style=\"font-weight:bold\""; } ?>>
        <a href="go_to_taskhistory.php?tkt=<?php echo $row_rs_list['tktin_idtktin']; ?>&amp;tktcat=<?php echo $row_rs_list['tktcategory_idtktcategory']; ?>&amp;title=<?php echo $row_rs_list['refnumber']; ?>&amp;tabview=1&amp;team_member=9&amp;keepThis=true&amp;TB_iframe=true&amp;height=415&amp;width=780&amp;inlineId=hiddenModalContent&amp;modal=true" id="thickBoxLink" class="thickbox" >
		<?php 
