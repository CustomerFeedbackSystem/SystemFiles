...
if($_SESSION['exportid']==1)
	...
	header("Content-Disposition: attachment; filename=tickets_received_per_category_regional.xls");
...
<div style="padding:10px">
	...
	FROM usrteamzone WHERE usrteam_idusrteam=".$_SESSION['MVGitHub_idacteam']." AND idusrteamzone=".$_SESSION['MVGitHub_userteamzoneid']." ORDER BY idusrteamzone ASC";
    $res=mysql_query($sql);
	...
	AND idusrteamzone=".$_SESSION['MVGitHub_userteamzoneid']." 
    ...
    if ($num > 0)
		...
		?>
        <td class="tbl_data">
            <?php if ($fet_data['tkts']>0) { echo $fet_data['tkts']; } else { echo "0"; }?>
		</td>
        <?php
