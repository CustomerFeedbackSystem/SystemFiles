...
$query_role = "SELECT idusrrole,usrrolename,userteamzonename,sysprofile,usrrole_idusrrole FROM usrrole INNER JOIN usrteamzone ON usrrole.usrteamzone_idusrteamzone=usrteamzone.idusrteamzone INNER JOIN usrteam ON usrteamzone.usrteam_idusrteam=usrteam.idusrteam  INNER JOIN sysprofiles ON usrrole.sysprofiles_idsysprofiles=sysprofiles.idsysprofiles LEFT JOIN usrac ON usrrole.idusrrole=usrac.usrrole_idusrrole WHERE ".$_SESSION['qry_filter_global']." AND  usrrole_idusrrole IS NULL ORDER BY usrrolename ASC";
...
<div style="padding:5px 0px 100px 0px">
    ...
    <table border="0" cellpadding="0" cellspacing="0" >
    	<tr>
           ...
           <input type="hidden" value="<?php echo $_SESSION['sec_mod'];?>" name="mod" />
            ...
            <input type="text" name="userac_find" value="" />
            </td>
            <td style="background-color:#f5f5f5"> <select name="zone" id="zone" onChange="getprofile(this.value)">
			   ...
			   if ($_SESSION['is_global_view']==1)
				...
				$fet_regions=mysql_fetch_array($res_regions);
					do {
					echo "<option ";
						if ( (isset($_SESSION['filter_region'])) && ($_SESSION['filter_region']==$fet_regions['idusrteamzone']) )
							{
    ...
    <div>
    	<div class="tab_area">
            ...
            <span class="tab_on"><a href="<?php echo $_SERVER['PHP_SELF'];?>?parentviewtab=4">Vacant Roles</a></span>
...
?>
    	<tr>
            ...
            <td class="tbl_h">Prior Used?</td>
            <td class="tbl_h">Profile</td>
            <td class="tbl_h">Tasks Pending</td>
          <td class="tbl_h">Assign User Account </td>
...
$prev_role = "&laquo; previous";
$next_role = "next &raquo;";
$separator = " | ";
$max_links = 10;
...
?>>
			...
			//tasks
			$res_tasks=mysql_query("SELECT count(*) as tasks_pending FROM wftasks WHERE ( (wftskstatustypes_idwftskstatustypes=6 AND wftskstatusglobal_idwftskstatusglobal=2) OR (wftskstatustypes_idwftskstatustypes=0 AND wftskstatusglobal_idwftskstatusglobal=1)) AND wftasks.usrrole_idusrrole=".$row_role['idusrrole']."");
			$fet_tasks=mysql_fetch_array($res_tasks);	
			
			if ($fet_tasks['tasks_pending']> 0 )
				{
			?>
             <a class="tooltip"><img src="../assets_backend/icons/icon_exc.gif" border="0" align="absmiddle"/><span>This role still has pending tasks in the profile. Please transfer these tasks to an active role</span></a>
             <?php
			 	}
			 ?>
            ...
            <td class="tbl_data">
              ...
              <td class="tbl_data">
              <?php
			  $res_old=mysql_query("SELECT idwftasks FROM wftasks WHERE usrrole_idusrrole=".$row_role['idusrrole']." OR sender_idusrrole=".$row_role['idusrrole']." LIMIT 1");
			  $fet_old=mysql_fetch_array($res_old);
			   if ($fet_old['idwftasks']>0) { echo "<span style=\"background-color:#009900; padding:2px; height:5px; width:10px;color:#ffffff\">Used &amp; Vacant</span>"; } else { echo "<span style=\"background-color:#ff0000;padding:2px; height:5px; width:10px; color:#ffffff\">Never Occupied</span>"; } ?>
              </td>
				<td class="tbl_data">
              <?php echo $row_role['sysprofile'];?>
              </td>
            <td class="tbl_data">
			<?php 	
			if ($fet_tasks['tasks_pending'] > 0 ) {  echo "<span style=\"color:#ffffff;background-color:#999999; padding:5px; font-weight:bold\">".$fet_tasks['tasks_pending']." </span><small><a href=\"pop_myteam_tasks_admin.php?tabview=2&team_member=".$row_role['idusrrole']."&keepThis=true&TB_iframe=true&height=560&width=1152&inlineId=hiddenModalContent&modal=true\" class=\"thickbox\"> Transfer Tasks</a></small>"; } else { echo "-"; }?>
            </td>
            <td class="tbl_data" valign="top"><?php echo $row_role['usrrole_idusrrole'];?> <small><a href="pop_assign_usertorole.php?roleid=<?php echo $row_role['idusrrole'];?>&amp;tabview=0&amp;keepThis=true&amp;TB_iframe=true&amp;height=<?php echo $_SESSION['tb_height'];?>&amp;width=<?php echo $_SESSION['tb_width'];?>&amp;inlineId=hiddenModalContent&amp;modal=true" class="thickbox"> Assign User </a></small></td>
