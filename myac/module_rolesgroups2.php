...
$query_role = "SELECT idusrrole,usrrolename,userteamzonename,sysprofile FROM usrrole  INNER JOIN usrteamzone ON usrrole.usrteamzone_idusrteamzone=usrteamzone.idusrteamzone INNER JOIN usrteam ON usrteamzone.usrteam_idusrteam=usrteam.idusrteam  INNER JOIN sysprofiles ON usrrole.sysprofiles_idsysprofiles=sysprofiles.idsysprofiles WHERE ".$_SESSION['qry_filter_global']." ORDER BY usrrolename ASC";
...
<div style="padding:5px 0px 100px 0px">
    ...
    <div>
		...
		<span class="tab_off"><a href="<?php echo $_SERVER['PHP_SELF'];?>?parentviewtab=2"><?php echo $lbl_groups;?></a></span></div>
    </div>
    </div>
    <div>
...
?>>
            ...
            <table border="0" cellpadding="0" cellspacing="0">
            	<tr>
                	<td class="text_body">
                    <small><a href="pop_assign_usertorole.php?roleid=<?php echo $row_role['idusrrole'];?>&amp;tabview=0&amp;keepThis=true&amp;TB_iframe=true&amp;height=<?php echo $_SESSION['tb_height'];?>&amp;width=<?php echo $_SESSION['tb_width'];?>&amp;inlineId=hiddenModalContent&amp;modal=true" class="thickbox"> <?php if ($num_aco > 0){ echo "Change"; } else { echo "Assign"; }?> User </a></small>
                    </td>
                    <td>
