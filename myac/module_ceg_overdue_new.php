...
*/
						/*	$sql_CEG_UPTO3="SELECT count(idtktinPK) AS CEG_IN,TIME_TO_SEC(TIMEDIFF('".$timenowis."',tktin.timereported)) as tsec FROM tktin 
							...
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') <= (129600+(tktcategory.tat/60))							
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'
							AND tktstatus_idtktstatus < 4";
						*/
						$sql_CEG_UPTO3 ="SELECT count(idtktin) AS CEG_IN FROM tktin
						INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
						WHERE 
						TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) >= 864000 
						AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) <7776000 
						AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'
						AND tktstatus_idtktstatus <4";
							
//echo $sql_CEG_UPTO3;
//AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',tktin.timereported)) <= (7776000+tktcategory.tat)
//SELECT TIMESTAMPDIFF(MINUTE,timereported,'2014-11-12 06:08:59'),timereported from tktin
//echo $sql_CEG_UPTO3;
...
/*							$sql_CEG_UPTO6="select count(idtktinPK) AS CEG_IN from tktin where DATEDIFF(NOW(),timedeadline)>90 AND DATEDIFF(NOW(),timedeadline) <=180
							...
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'";
*/					
							/*$sql_CEG_UPTO6="SELECT count(idtktinPK) AS CEG_IN FROM tktin 
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE timeclosed='0000-00-00 00:00:00'
							AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',tktin.timereported)) > tktcategory.tat
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') > (129600+(tktcategory.tat/60))														
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') <= (259200+(tktcategory.tat/60))														
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'
							AND tktstatus_idtktstatus < 4";
*/
							$sql_CEG_UPTO6 ="SELECT count(idtktin) AS CEG_IN FROM tktin
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE 
							TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) >= 7776000 
							AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) <15552000 
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'
							AND tktstatus_idtktstatus <4";
...
/*							$sql_CEG_UPTO9="select count(idtktinPK) AS CEG_IN from tktin where DATEDIFF(NOW(),timedeadline)>180 AND DATEDIFF(NOW(),timedeadline) <=270
							...
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'";
*/
						/*	$sql_CEG_UPTO9="SELECT count(idtktinPK) AS CEG_IN FROM tktin 
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE timeclosed='0000-00-00 00:00:00'
							AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',tktin.timereported)) > tktcategory.tat
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') > (259200+(tktcategory.tat/60))														
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') <= (388800+(tktcategory.tat/60))														
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'
							AND tktstatus_idtktstatus < 4";
							*/
							
							$sql_CEG_UPTO9 ="SELECT count(idtktin) AS CEG_IN FROM tktin
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE 
							TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) >= 15552000 
							AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) < 23328000
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'
							AND tktstatus_idtktstatus <4";
							...
							$total_UPTO9=$total_UPTO9+$fet_CEG_UPTO9['CEG_IN'];
...
/*							$sql_CEG_UPTO12="select count(idtktinPK) AS CEG_IN from tktin where DATEDIFF(NOW(),timedeadline)>270 AND DATEDIFF(NOW(),timedeadline) <=360
							...
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'";
*/
					/*		$sql_CEG_UPTO12="SELECT count(idtktinPK) AS CEG_IN FROM tktin 
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE timeclosed='0000-00-00 00:00:00'
							AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',tktin.timereported)) > tktcategory.tat
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') > (388800+(tktcategory.tat/60))														
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') <= (518400+(tktcategory.tat/60))														
							AND tktin.usrteamzone_idusrteamzone='".$fet_region['idusrteamzone']."'
							AND tktstatus_idtktstatus < 4";
							*/
							$sql_CEG_UPTO12 ="SELECT count(idtktin) AS CEG_IN FROM tktin 
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE 
							TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) > 23328000 
							AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) <=31104000 
							AND tktstatus_idtktstatus <4";
...
/*							$sql_CEG_BEYOND12="select count(idtktinPK) AS CEG_IN from tktin where DATEDIFF(NOW(),timedeadline)>360
							...
							/*$sql_CEG_BEYOND12="SELECT count(idtktinPK) AS CEG_IN FROM tktin 
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE timeclosed='0000-00-00 00:00:00'
							AND TIME_TO_SEC(TIMEDIFF('".$timenowis."',tktin.timereported)) > tktcategory.tat
							AND TIMESTAMPDIFF(MINUTE,tktin.timereported,'".$timenowis."') > (518400+(tktcategory.tat/60))																					
							...
							AND tktstatus_idtktstatus < 4";
							*/
							$sql_CEG_BEYOND12 ="SELECT count(idtktin) AS CEG_IN FROM tktin 
							INNER JOIN tktcategory ON tktin.tktcategory_idtktcategory=tktcategory.idtktcategory 
							WHERE 
							TIME_TO_SEC(TIMEDIFF('".$timenowis."',(DATE_ADD(timereported, INTERVAL (tat/86400) DAY)))) > 31104000 
							AND tktstatus_idtktstatus <4";
        	...
        	<td style="padding:5px 0px 0px 0px">           
            	...
            	<div style="padding:10px 0px">
          		<table border="0" width="100%" cellpadding="2" cellspacing="0">            	
                    ...
                    <tr>
                        <td class="table_header" style="font-size:11px; font-weight:bold">
							<a href="">&laquo; Back to main</a></td>                       
                        </td>
                       	<td class="table_header" style="font-size:11px; font-weight:bold">
                        Up to 3 Months
                       	</td>
           	...
           	<div style="padding:10px 0px">
          		<table border="0" width="100%" cellpadding="2" cellspacing="0">            	
                    ...
                    <tr>
						...
						$sql_task="SELECT idwftasks FROM reports_wftasks 
						WHERE tktin_idtktin=".$fet_ticket['idtktinPK']." 
						AND ((reports_wftasks.wftskstatustypes_idwftskstatustypes=0 AND reports_wftasks.wftskstatusglobal_idwftskstatusglobal=1) OR (reports_wftasks.wftskstatustypes_idwftskstatustypes=6 AND reports_wftasks.wftskstatusglobal_idwftskstatusglobal=2))
